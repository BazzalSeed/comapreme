/**
 * Chinese New Year (Spring Festival) dates as [month, day] (month is 1-12).
 *
 * The Chinese zodiac year begins on Chinese New Year, NOT January 1. Someone born
 * before that year's CNY belongs to the previous zodiac animal. This lookup powers
 * that boundary correction in `getZodiac`.
 *
 * Range: 1924–2044 (covers all plausible users). Outside this range we approximate
 * with a fixed Feb-4 (Lichun) boundary — see `getCnyBoundary`.
 *
 * Source: well-established astronomical Spring Festival dates.
 */
...
export const CNY_MIN_YEAR = 1924;
export const CNY_MAX_YEAR = 2044;

/**
 * The CNY boundary for a given Gregorian year as [month, day]. Within the known
 * range, the exact date; outside it, an approximation (Feb 4 / Lichun).
 */
export function getCnyBoundary(year: number): { month: number; day: number; approximate: boolean } {
  const known = CNY_DATES[year];
  if (known) return { month: known[0], day: known[1], approximate: false };
